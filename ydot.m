...
function ydot = ydot(t, y, W)
    len = 12; % Bredde på brua
    a = 0.2; % Hookes koeffisient for ulinearitet
    d = 0.01; % Dempingskoeffisient
    omega = 2 * pi * 38 / 60; % Vinkel på brua
    
    % Eksponentledd
    e1 = exp(a * (y(1) - 0.5*len * sin(y(3)))); % e^a * (y(1) - 0.5*len * sin(y(3)))
    e2 = exp(a * (y(1) + 0.5*len * sin(y(3)))); % e^a * (y(1) + 0.5*len * sin(y(3)))
    ...
    ydot(3) = y(4);
    % ?''
    ydot(4) = -d * y(4) + 1.2 * cos(y(3)) * (e1 - e2) / (0.5*len * a);
end
