...
function [ Wout , E ] = fehlbergstep( t, y, h )
   ...
   B=[ 25/216 0 1408/2565  2197/4104   -1/5  0;
    ...
    s1=ydot(t,y);
    s2=ydot(t+h/4,y+h*(A(1,1)*s1));
    s3=ydot(t+3*h/8,y+h*(A(2,1)*s1+A(2,2)*s2));
    s4=ydot(t+12*h/13,y+h*(A(3,1)*s1+A(3,2)*s2+A(3,3)*s3));
    s5=ydot(t,y+h*(A(4,1)*s1+A(4,2)*s2+A(4,3)*s3+A(4,4)*s4));
    s6=ydot(t+h/2,y+h*(A(5,1)*s1+A(5,2)*s2+A(5,3)*s3+A(5,4)*s4+A(5,5)*s5));
